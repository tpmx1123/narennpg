    ...
    <section className="mb-8 lg:mb-10">
      ...
      >
        ...
        {GACHIBOWLI_TWENTY.paragraphs.map((para) => (
          <p
            key={para.slice(0, 48)}
            className="text-sm sm:text-[15px] text-brand-charcoal-light leading-relaxed text-center"
          >
            <ContactText>{para}</ContactText>
          </p>
        ))}
        <p className="text-center text-sm sm:text-[15px] text-brand-charcoal font-display font-semibold pt-2">
          Prefer to message? <PhoneWhatsAppLinks className="text-brand-burgundy font-semibold hover:underline underline-offset-2" />
        </p>
      </motion.div>
    </section>
  );
