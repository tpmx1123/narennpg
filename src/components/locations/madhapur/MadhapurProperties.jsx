    ...
    <section className="mb-8 sm:mb-10 lg:mb-12 py-8 sm:py-10 lg:py-12 -mx-4 sm:-mx-6 lg:-mx-10 px-4 sm:px-6 lg:px-10 bg-brand-green-pale/35 border-y border-brand-gold/15">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6 max-w-5xl mx-auto">
        {MADHAPUR_PROPERTIES.items.map((property) => (
          <article
            key={property.name}
            className="bg-white rounded-2xl border border-brand-gold/20 p-3.5 sm:p-5 shadow-sm min-w-0 overflow-hidden"
          >
            <h3 className="font-display font-bold text-base sm:text-lg text-brand-green mb-3 sm:mb-4">
              {property.name}
            </h3>
