...
import { HITECH_CITY_PROPERTIES } from '../../../data/hitechCityLocationData';
import { PHONE_DISPLAY, PHONE_TEL } from '../../../data/sitePages';

const EASE = [0.16, 1, 0.3, 1];

export default function HitechCityProperties() {
  return (
      ...
      >
        <div className="lg:col-span-5 space-y-5">
          <span className="text-[11px] font-bold tracking-[0.25em] text-brand-gold uppercase block">
            {HITECH_CITY_PROPERTIES.eyebrow}
          </span>
          <h2 className="text-3xl sm:text-4xl lg:text-4xl font-display font-bold text-brand-green tracking-tight leading-[1.15]">
            Our{' '}
            <span className="text-brand-gold italic font-medium">
              {HITECH_CITY_PROPERTIES.h2Accent}
            </span>
          </h2>
          <p className="text-sm sm:text-[15px] text-brand-charcoal-light leading-relaxed">
            {HITECH_CITY_PROPERTIES.body}
          </p>
          <address className="not-italic text-sm font-display font-semibold text-brand-green leading-relaxed">
            {HITECH_CITY_PROPERTIES.streetAddress}
            <br />
            {HITECH_CITY_PROPERTIES.locality}, {HITECH_CITY_PROPERTIES.region}{' '}
            {HITECH_CITY_PROPERTIES.postalCode}
          </address>
          <a
            href={`tel:${PHONE_TEL}`}
