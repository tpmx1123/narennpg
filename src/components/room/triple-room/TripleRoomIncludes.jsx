...
import { TRIPLE_SHARING_INCLUDES } from '../../../data/tripleSharingData';

const EASE = [0.22, 1, 0.36, 1];

export default function TripleRoomIncludes() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm sm:text-[15px] text-brand-charcoal-light leading-relaxed mb-6">
        A triple sharing room is designed so all three residents have their own comfortable, private
        space within a shared room:
      </p>
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
        {TRIPLE_SHARING_INCLUDES.map((item, i) => (
          <motion.li
            key={item}
