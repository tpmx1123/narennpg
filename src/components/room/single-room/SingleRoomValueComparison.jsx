...
import { SINGLE_SHARING_COMPARISON } from '../../../data/singleSharingData';
...
export default function SingleRoomValueComparison() {
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 mb-8 lg:mb-10 items-end">
        <div className="lg:col-span-5">
          ...
          <h2 className="font-display font-bold text-2xl sm:text-3xl lg:text-[2.15rem] text-brand-green tracking-tight leading-[1.12]">
            Single Sharing vs Renting a{' '}
            <span className="text-brand-gold italic font-medium">1BHK in Madhapur</span>
          </h2>
        </div>
        <div className="lg:col-span-7">
          <p className="text-sm sm:text-[15px] text-brand-charcoal-light leading-relaxed border-l-[3px] border-brand-burgundy pl-4 lg:pl-5">
            {SINGLE_SHARING_COMPARISON.intro}
          </p>
        </div>
      </div>
      ...
      >
        {SINGLE_SHARING_COMPARISON.points.map((point, index) => (
          <motion.li
            key={point}
            custom={index}
