...
import { QUAD_SHARING_INCLUDES } from '../../../data/quadSharingData';

const EASE = [0.22, 1, 0.36, 1];

export default function QuadRoomIncludes() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm sm:text-[15px] text-brand-charcoal-light leading-relaxed mb-6">
        A four sharing room is designed so all four residents have their own comfortable, private
        space within a shared room:
      </p>
      <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
        {QUAD_SHARING_INCLUDES.map((item, i) => (
          <motion.li
            key={item}
